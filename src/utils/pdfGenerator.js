...
export const generateCalendarPDF = async (options) => {
  ...
  try {
    ...
    await generateOfficialPages(pdf, tipoFiltro, año, periodo, pdfWidth, pdfHeight);
 ...
 */
const generateOfficialPages = async (pdf, tipoFiltro, año, periodo, pdfWidth, pdfHeight) => {
  let mesesAGenerar = [];
  ...
  console.log(`Meses a generar:`, mesesAGenerar.map(m => dayjs().year(año).month(m-1).format('MMMM')));
  ...
  for (let i = 0; i < mesesAGenerar.length; i++) {
    ...
    console.log(`🔄 Generando página ${i + 1}/${mesesAGenerar.length} - ${dayjs().year(año).month(mes-1).format('MMMM YYYY')}`);
    ...
    await generateOfficialPage(pdf, año, mes, tipoFiltro, periodo, pdfWidth, pdfHeight);
    
    console.log(`✅ Página completada: ${dayjs().year(año).month(mes-1).format('MMMM YYYY')}`);
 ...
 */
const generateOfficialPage = async (pdf, año, mes, tipoFiltro, periodo, pdfWidth, pdfHeight) => {
  // Colores oficiales
  ...
  // Encabezado oficial
  drawOfficialHeader(pdf, año, mes, tipoFiltro, periodo, pdfWidth, blanco, azulOficial);
 ...
 */
const drawOfficialHeader = (pdf, año, mes, tipoFiltro, periodo, pdfWidth, blanco, azulOficial) => {
  ...
  pdf.text('Consejo Superior de la Judicatura', rightSideX, 12, { align: 'center' });
  pdf.text('Consejo Seccional de la Judicatura de Nariño', rightSideX, 20, { align: 'center' });
...
const debugCalendarioElementos = () => {
  ...
  todosLosBotones.forEach((btn, index) => {
    ...
    console.log(`Botón ${index}: "${texto}" | Title: "${title}" | Clases: "${clases}"`);
  ...
  selectoresAnterior.forEach((selector, index) => {
    try {
      ...
      if (selector.includes('contains')) {
        elemento = Array.from(document.querySelectorAll('button')).find(btn => 
          btn.textContent.trim().toLowerCase() === 'anterior'
        );
      } else {
        elemento = document.querySelector(selector);
      }
      
      console.log(`Selector ${index + 1} "${selector}": ${elemento ? '✅ ENCONTRADO' : '❌ NO ENCONTRADO'}`);
      if (elemento) {
        console.log(`  Texto: "${elemento.textContent.trim()}" | Clases: "${elemento.className}"`);
      }
    } catch (error) {
...
const drawCalendarContent = async (pdf, año, mes, pdfWidth, pdfHeight) => {
  try {
    console.log(`📸 Iniciando captura para ${dayjs().year(año).month(mes-1).format('MMMM YYYY')}`);
    
    // Ejecutar debugging si es el primer mes
    if (mes === 7) { // Solo para julio (primer mes del segundo semestre)
    ...
    console.log(`✅ Elemento calendario encontrado: ${calendarElement.className}`);

    // Verificar que el calendario muestra el mes correcto
    const mesTexto = dayjs().year(año).month(mes-1).format('MMMM').toLowerCase();
    const contenidoCalendario = calendarElement.textContent.toLowerCase();
    
    if (contenidoCalendario.includes(mesTexto) || contenidoCalendario.includes(mes.toString())) {
      console.log(`✅ Calendario muestra el mes correcto: ${mesTexto}`);
    } else {
      console.log(`⚠️ Posible problema: calendario podría no mostrar ${mesTexto}`);
      console.log(`Contenido encontrado: ${contenidoCalendario.substring(0, 200)}...`);
    }
...
const detectarMesActualCalendario = () => {
  try {
    ...
    for (let i = 0; i < mesIndicadores.length; i++) {
      ...
      console.log(`🔎 Analizando indicador ${i + 1}: "${texto.substring(0, 50)}..."`);
      ...
      for (let j = 0; j < meses.length; j++) {
        if (texto.includes(meses[j])) {
          console.log(`✅ Mes detectado: ${meses[j]} (${j + 1}) en indicador "${texto}"`);
      ...
      if (coincidenciasMes) {
        for (const coincidencia of coincidenciasMes) {
          ...
          if (numeroMes >= 1 && numeroMes <= 12) {
            console.log(`✅ Mes detectado por número: ${numeroMes} en "${texto}"`);
            return numeroMes;
          }
        }
      }
    ...
    const calendarioElement = document.querySelector('.rbc-calendar') || 
                             document.querySelector('.calendar-container') ||
                             document.querySelector('[class*="calendar"]');
...
const encontrarBotonPorTexto = (textoBoton) => {
  ...
  let botonEncontrado = botones.find(btn => 
    btn.textContent.trim().toLowerCase() === textoBoton.toLowerCase()
  );
  ...
  if (botonEncontrado) {
    console.log(`✅ Botón "${textoBoton}" encontrado por texto parcial: "${botonEncontrado.textContent.trim()}"`);
    return botonEncontrado;
  }
...
const navegarAMesEspecifico = async (año, mes) => {
  try {
    console.log(`🔄 Navegando calendario a: ${dayjs().year(año).month(mes-1).format('MMMM YYYY')}`);
    
    // Verificar si ya estamos en el mes correcto
    const mesActual = detectarMesActualCalendario();
    if (mesActual === mes) {
      console.log(`✅ Ya estamos en ${dayjs().year(año).month(mes-1).format('MMMM')}, no es necesario navegar`);
      return;
    }
    ...
    console.log(`📍 Mes actual detectado: ${dayjs().month(mesBaseActual-1).format('MMMM')} (${mesBaseActual})`);
    console.log(`🎯 Mes objetivo: ${dayjs().year(año).month(mes-1).format('MMMM YYYY')} (${mes})`);
    console.log(`📊 Diferencia calculada: ${diferenciaMeses} meses`);

    // Buscar botones usando la función auxiliar (más confiable)
    const nextButton = encontrarBotonPorTexto('Siguiente') ||
                      document.querySelector('.rbc-btn-group button:last-child') || 
                      document.querySelector('[aria-label="next"]');
    
    const prevButton = encontrarBotonPorTexto('Anterior') ||
                      document.querySelector('.rbc-btn-group button:first-child') || 
                      document.querySelector('[aria-label="previous"]');

    console.log(`🔍 Botones encontrados:`);
    console.log(`  - Siguiente: ${!!nextButton} ${nextButton ? `"${nextButton.textContent.trim()}"` : ''}`);
    console.log(`  - Anterior: ${!!prevButton} ${prevButton ? `"${prevButton.textContent.trim()}"` : ''}`);
    ...
    if (diferenciaMeses > 0 && nextButton) {
      ...
      for (let i = 0; i < diferenciaMeses; i++) {
        console.log(`📱 Clic ${i + 1}/${diferenciaMeses} en botón SIGUIENTE: "${nextButton.textContent.trim()}"`);
        nextButton.click();
        ...
        if ((i + 1) % 2 === 0) {
          ...
          console.log(`📈 Progreso: ahora en ${dayjs().month((mesIntermedio||1)-1).format('MMMM')} (${mesIntermedio})`);
        }
      }
      ...
      console.log(`🎯 Vamos a hacer ${clicsNecesarios} clics en el botón: "${prevButton.textContent.trim()}"`);
      
      for (let i = 0; i < clicsNecesarios; i++) {
        console.log(`📱 Clic ${i + 1}/${clicsNecesarios} en botón ANTERIOR: "${prevButton.textContent.trim()}"`);
        ...
        console.log(`⏳ Esperando ${700}ms para que el calendario se actualice...`);
        await new Promise(resolve => setTimeout(resolve, 700));
        
        // Verificar progreso después de cada navegación
        const mesIntermedio = detectarMesActualCalendario();
        const nombreMesIntermedio = dayjs().month((mesIntermedio||1)-1).format('MMMM');
        console.log(`📉 Progreso clic ${i + 1}: ahora en ${nombreMesIntermedio} (${mesIntermedio})`);
        
        // Si el mes no cambió, intentar nuevamente
        if (i > 0 && mesIntermedio === mesBaseActual) {
          console.log(`⚠️ El mes no cambió desde el inicio, intentando clic adicional...`);
    ...
    if (mesFinal === mes) {
      console.log(`✅ 🎉 NAVEGACIÓN EXITOSA a ${dayjs().year(año).month(mes-1).format('MMMM YYYY')}`);
    } else {
      console.log(`⚠️ POSIBLE PROBLEMA: esperaba mes ${mes} (${dayjs().month(mes-1).format('MMMM')}), pero detecté mes ${mesFinal} (${dayjs().month((mesFinal||1)-1).format('MMMM')})`);
    }
...
const drawCalendarPlaceholder = (pdf, año, mes, pdfWidth, pdfHeight) => {
  ...
  pdf.setFontSize(18);
  pdf.setFont('helvetica', 'bold');
  const mesNombre = dayjs().year(año).month(mes - 1).format('MMMM YYYY');
  pdf.text(mesNombre, pdfWidth / 2, calendarY + 30, { align: 'center' });
  
  // Mensaje
  pdf.setFontSize(12);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Calendario generado automáticamente', pdfWidth / 2, calendarY + 50, { align: 'center' });
  pdf.text('Ver aplicación web para contenido detallado', pdfWidth / 2, calendarY + 65, { align: 'center' });
};
...
const drawOfficialFooter = (pdf, pdfWidth, pdfHeight, azulOficial) => {
  ...
  pdf.text(`Generado el ${fechaGeneracion} - Habeas Corpus`, pdfWidth / 2, footerY + 8, { align: 'center' });
};
...
const generateFileName = (tipoFiltro, año, periodo) => {
  ...
  switch (tipoFiltro) {
    ...
    case 'semestre':
      const semestreTexto = periodo === 1 ? 'PrimerSemestre' : 'SegundoSemestre';
      return `${baseFileName}_${semestreTexto}${año}.pdf`;
    
    case 'trimestre':
      const trimestreTexto = ['PrimerTrimestre', 'SegundoTrimestre', 'TercerTrimestre', 'CuartoTrimestre'][periodo - 1];
...
const generateTitle = (tipoFiltro, año, periodo) => {
  ...
  switch (tipoFiltro) {
    ...
    case 'semestre':
      const semestreTexto = periodo === 1 ? 'Primer Semestre' : 'Segundo Semestre';
      return `${baseTitle} - ${semestreTexto} ${año}`;
    
    case 'trimestre':
      const trimestreTexto = ['Primer Trimestre', 'Segundo Trimestre', 'Tercer Trimestre', 'Cuarto Trimestre'][periodo - 1];
