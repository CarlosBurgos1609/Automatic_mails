...
export const getJuzgadoStatsInTimeRange = (juzgadoId, turnos, timeRange) => {
  const turnosJuzgado = turnos.filter(turno => turno.juzgado_id === juzgadoId);
...
export const getJuzgadoTemporalStatusInRange = (juzgado, turnos, timeRange, referenceDate = null) => {
  const hoy = referenceDate ? dayjs(referenceDate) : dayjs();
  const turnosJuzgado = turnos.filter(turno => turno.juzgado_id === juzgado.id);
  ...
  const proximoTurno = turnosOrdenados.find(turno => 
    turno.fecha.isAfter(hoy) || turno.fecha.isSame(hoy, 'day')
  );
  ...
  const turnosPasados = turnosOrdenados.filter(turno => turno.fecha.isBefore(hoy));
...
export const calculateTemporalStats = (juzgados, turnos, timeRange, referenceDate = null) => {
  const hoy = referenceDate ? dayjs(referenceDate) : dayjs();
...
export const isFutureInRange = (date, timeRange, referenceDate = null) => {
  ...
  return checkDate.isAfter(reference) && 
